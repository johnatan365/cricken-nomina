...
type WorkerWithRates = Worker & { rates: HourlyRate[]; schedules: WorkerSchedule[]; has_cash_register: boolean; sunday_rate: number | null; has_kitchen_access: boolean; has_cash_order_access: boolean; has_food_tracker_access: boolean }
...
export default function TrabajadoresPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const ws = workersData.map((w) => ({
      ...
      has_food_tracker_access: (w as WorkerWithRates).has_food_tracker_access ?? false,
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          {selected ? (
            <div className="bg-white/10 rounded-3xl border border-white/10 p-5 space-y-4">
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-white/5 rounded-2xl border border-white/10">
                <div>
                  <p className="text-white text-sm font-semibold">🍔 Food Tracker</p>
                  <p className="text-white/40 text-xs mt-0.5">
                    {selected.has_food_tracker_access ? 'Puede hacer pedidos Food Tracker' : 'Sin acceso a Food Tracker'}
                  </p>
                </div>
                <button onClick={async () => {
                  ...
                  setWorkers(prev => prev.map(w => w.id === selected.id ? { ...w, has_food_tracker_access: newVal } : w))
                  setSelected((prev: WorkerWithRates | null) => prev ? { ...prev, has_food_tracker_access: newVal } : prev)
                  showStatus('success', newVal ? 'Food Tracker activado' : 'Food Tracker desactivado')
                }}
                  className={`relative w-12 h-6 rounded-full transition-colors duration-200 flex-shrink-0 ${selected.has_food_tracker_access ? 'bg-yellow-400' : 'bg-white/20'}`}>
                  <span className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${selected.has_food_tracker_access ? 'left-7' : 'left-1'}`} />
                </button>
              </div>
