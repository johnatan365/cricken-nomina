...
export async function GET(req: NextRequest) {
  ...
  const { data: order } = await supabase.from('kitchen_orders')
    .select('*, items:kitchen_order_items(*, product:kitchen_products(*))')
    .eq('delivery_date', deliveryDate).maybeSingle()
  const { data: products } = await supabase.from('kitchen_products')
...
export async function POST(req: NextRequest) {
  ...
  const supabase = createAdminClient()
  const { data: existing } = await supabase.from('kitchen_orders')
    .select('id').eq('delivery_date', delivery_date).maybeSingle()
  if (existing) return NextResponse.json({ error: 'Ya existe un pedido para ese día' }, { status: 409 })

  const { data: order, error } = await supabase.from('kitchen_orders')
    .insert({ worker_id, delivery_date, status: 'pending' }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })

  const orderItems = items
    .filter((i: {qty_requested: number}) => i.qty_requested > 0)
    .map((i: {product_id: string; qty_requested: number}) => ({ order_id: order.id, product_id: i.product_id, qty_requested: i.qty_requested }))
  if (orderItems.length > 0) await supabase.from('kitchen_order_items').insert(orderItems)

  // WhatsApp CallMeBot
  const apiKey = process.env.CALLMEBOT_API_KEY || ''
  if (apiKey) {
    const lines = items.filter((i: {qty_requested: number}) => i.qty_requested > 0)
      .map((i: {name: string; qty_requested: number}) => `• ${i.name}: ${i.qty_requested}`).join('\n')
...
export async function PATCH(req: NextRequest) {
  ...
  for (const d of deliveries) {
    await supabase.from('kitchen_order_items')
      ...
      .eq('order_id', order_id).eq('product_id', d.product_id)
  }
